...
describe('no employer-letter shift (removed 2026-09-26)', () => {
  // The backtest scored the shift against 7,112 real decisions and it ADDED
  // error (3.9 -> 6.5 days typical miss). The engine takes no letter input
  // and reports no letter term, so no surface can reintroduce it quietly.
  it('reports no letter term on the result', () => {
    const r = estimateQueueDecision({
      filingDate: '2025-12-15',
      today: '2026-09-26',
      frontier: null,
      cohorts: [],
    });
    expect('letterDeltaDays' in r).toBe(false);
  });
});

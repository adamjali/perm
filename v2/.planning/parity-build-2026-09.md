...
- [x] 12 Suites green by project (unit+isolated 3,160; components 2,174; convex 1,121; ssr 11); three production builds green; on the third: page audit 0/68, glued 0/68, visibility 60/60 clean (the full 275-page run stalls on the local server; sampled), registration gate 0. Lighthouse mobile local: /lca-wages 31 vs the older /tools/salary-explorer 38 as control, same class (TBT 6.9s vs 8.0s on this throttled Mac; CLS 0.24 vs 0.30, both from the results area filling in after fetch and pushing the FAQ down). NEXT: reserve the explorer results' height while loading, on both pages. Deploy authorized by Adam 2:50 AM ET Sep 8 ("commit all push all deploy all")

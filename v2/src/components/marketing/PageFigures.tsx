...
/**
 * A case's dates on one track.
 *
 * The sign-up page's whole claim is that the deadlines are computed rather
 * than remembered, so the figure is the track those dates sit on: a filing
 * window that opens and closes, a wage expiry that can cut it short, and the
 * marks in between. It is schematic on purpose - a real case's dates belong to
 * that case, and inventing plausible ones here would be a fake screenshot.
 */
export function DeadlineTrackFigure({ className }: { className?: string }) {
  const marks = [
    { x: 40, label: "PWD" },
    { x: 108, label: "Recruit" },
    { x: 176, label: "File" },
  ...
  return (
    <svg
      ...
      className={className}
      role="img"
      aria-label="A PERM case's dates laid along one track, with the filing window shaded"
    >
      {/* The filing window: the span the whole product exists to get right. */}
      <rect x="108" y="34" width="68" height="26" fill="var(--data-good-ink)" fillOpacity="0.18" />
      <line x1="108" y1="30" x2="108" y2="64" stroke="var(--data-good-ink)" strokeWidth="2" />
      <line x1="176" y1="30" x2="176" y2="64" stroke="var(--data-good-ink)" strokeWidth="2" />

      {/* The track. */}
      <line x1="16" y1="47" x2="284" y2="47" stroke="currentColor" strokeWidth="2" />

      {marks.map((m) => (
        <g key={m.label}>
          <rect x={m.x - 5} y="42" width="10" height="10" fill="currentColor" />
          <text
            x={m.x}
            y="76"
            textAnchor="middle"
      ...
      {/* The cap. A wage determination expiring closes the window early, and
          it is the rule a hand-kept calendar misses most often. */}
      <line
        x1="150"
        y1="92"
        x2="150"

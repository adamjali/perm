/**
 * The social card for one Law firm page, generated from the entity's own row.
...
export const alt = "Law firm PERM filings, from DOL's disclosure files";
export const size = ENTITY_OG_SIZE;
export const contentType = "image/png";

export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const found = await resolveEntity("attorney", slug);
  if (!found) return new Response("Not found", { status: 404 });
  return generateEntityOG("attorney", found.row);
}

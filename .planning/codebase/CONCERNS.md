...
- **Files:**
  - `v2/src/instrumentation-client.ts` — hosts `posthog.init()` in a try/catch. (It also hosted `initBotId()` until 2026-09-23, when BotID moved to `components/security/BotIdInit.tsx` in the (authenticated) layout, because it guards only the signed-in chat.)
...
- **Rule:** client instrumentation that must run on every page lives in the single `src/instrumentation-client.ts`; never create a root-level one. Anything that only one area needs (BotID, since 2026-09-23) starts in that area's layout instead. `NEXT_PUBLIC_*` vars are build-time inlined, so analytics/observability changes need a redeploy to take effect.
